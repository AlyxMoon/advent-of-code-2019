  ...
  const changeSelectedMenuItem = async (dy = 0) => {
    ...
    if (activeOption + dy >= 0 && activeOption + dy < menuOptions.length) {
      ...
      return Promise.resolve()
        ...
        .then(() => activeConsole.write(`-- ${menuOptions[activeOption - dy].name}`, null, { noNewline: true }))
        .then(() => activeConsole.moveAndOrClearLine(dy, false))
        .then(() => activeConsole.write(`-- ${menuOptions[activeOption].name}`, null, { highlight: true, noNewline: true }))
