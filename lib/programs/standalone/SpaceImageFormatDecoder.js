...
const spaceImageGenerator = (spaceImage = [], width = 0, height = 0) => {
  ...
  return '\n' + reversedLayers
    ...
    .map((pixel, i) => {
      let newPixel = pixel
      // Can't get the terminal.str functionality to work, so manually entering in colors
      if (pixel === 0) newPixel = '^#^K ^:'
      if (pixel === 1) newPixel = '^#^c ^:'
      if (pixel === 2) newPixel = '^#^w ^:'
      if (pixel === 3) newPixel = terminal.str.bgWhite(' ')
      if (pixel === 4) newPixel = terminal.str.bgRed(' ')
