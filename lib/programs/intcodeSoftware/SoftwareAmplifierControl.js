const { generateUniqueValueArrayOfPermutations } = require('../../util')

class SoftwareAmplifierControl {
  #amplifierCount
  #phaseRange
  #phaseSequence
  #program
  ...
  run (computer) {
    ...
    if (this.#phaseSequence) {
      ...
      if (this.#phaseSequence.every(phase => phase >= 5 && phase <= 9)) {
        processes.forEach(process => process.setProgramFlags({ pauseOnWrite: true }))
    ...
    if (!feedbackMode) {
      ...
      while (processes.some(process => !process.isProgramFinished())) {
        processes.forEach((process, i) => {
          ...
          if (!process.isProgramFinished()) process.run()
        })
      }
    }
