...
export const fillArrayToIndex = (arr, index, val = 0) => {
  if (!Array.isArray(arr)) {
    throw new Error(`Invalid array passed to fillArrayToIndex: ${arr}`)
  }

  if (!Number.isInteger(index) || index < 0) {
    throw new Error(`Invalid index passed to fillArrayToIndex: ${index}, should be positive integer`)
  }
...
export class IntcodeProcess {
  ...
  run () {
    ...
    for (
      ...
      inputIndexes.forEach(index => fillArrayToIndex(program, index))
